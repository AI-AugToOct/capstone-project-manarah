...
async def healthz():
    ...
    return {"status": "ok", "service": "manarah-backend"}
...
async def get_content_status(content_id: str, user_id: str):
    ...
    try:
        ...
        # If completed, include decision in simplified format
        if status == "completed":
            decision = load_decision(content_id)
            if decision:
                # Get main violation category
                categories = decision.get("evidence_summary", {}).get("categories", [])
                main_violation = categories[0] if categories else "none"

                # Add decision fields to response while keeping status field
                response["status"] = "completed"
                response["decision"] = decision.get("action", "allow")
                response["main_violation_type"] = main_violation
                response["overall_confidence_score"] = round(decision.get("combined_score", 0) * 100, 1)
                response["vision_analysis_score"] = round(decision.get("vision_score", 0) * 100, 1)
                response["audio_analysis_score"] = round(decision.get("text_score", 0) * 100, 1)
                response["reason"] = decision.get("reasoning", "")
...
async def get_content_details(content_id: str, user_id: str):
    ...
    try:
        ...
        # Extract violations for cleaner display
        ...
        # Vision violations
        for v in vision_analysis.get("violations", []):
        ...
        # Text violations
        for v in text_analysis.get("violations", []):
            all_violations.append({
                "source": "text",
                "category": v.get("category", "unknown"),
                "severity": v.get("severity", "unknown"),
        ...
        response = {
            ...
            "uploaded_at": metadata.get("upload_timestamp"),
            "processed_at": decision.get("decision_timestamp"),
            
            # ✨ CLEAR DECISION SECTION
            "decision": {
                "action": decision.get("action"),
                "action_display": {
                    "auto_remove": "🚫 AUTO REMOVE - Content violates policies",
                    "human_review": "⚠️ REQUIRES HUMAN REVIEW",
                    "warning": "⚡ WARNING - Borderline content",
                    "approved": "✅ APPROVED - No violations"
                }.get(decision.get("action"), decision.get("action")), # type: ignore
                "priority": decision.get("priority", "normal"),
                "reasoning": decision.get("reasoning", ""),
                "combined_score": f"{decision.get('combined_score', 0) * 100:.1f}%",
                "vision_score": f"{decision.get('vision_score', 0) * 100:.1f}%",
                "text_score": f"{decision.get('text_score', 0) * 100:.1f}%"
            },
            
            # ✨ CLEAR VIOLATIONS SECTION
            "violations": {
                "total_count": decision.get("evidence_summary", {}).get("total_violations", 0),
                "has_violations": len(all_violations) > 0,
                "categories": decision.get("evidence_summary", {}).get("categories", []),
                "severity_breakdown": decision.get("evidence_summary", {}).get("severity_breakdown", {}),
            ...
            "analysis_summary": {
                ...
                "total_frames": vision_analysis.get("total_frames", 0),
                "transcript": text_analysis.get("transcript", ""),
                "caption": text_analysis.get("caption", ""),
                "language": text_analysis.get("language", "unknown"),
                "vision_violations": decision.get("evidence_summary", {}).get("vision_violations", 0),
                "text_violations": decision.get("evidence_summary", {}).get("text_violations", 0)
            },
